...
def enhance_doc_image(img_bgr: np.ndarray) -> np.ndarray:
    """
    Enhance a document/card photo for better face embedding quality.

    Aadhaar / PAN cards are often low-quality prints scanned or photographed:
    - Faded, low contrast
    - JPEG-compressed multiple times
    - Under/over-exposed

    Steps:
      1. Upscale small images so InsightFace has enough pixels
      2. CLAHE on the L-channel (perceptual contrast boost, avoids blowout)
      3. Unsharp mask for edge/detail recovery
    """
    if img_bgr is None or img_bgr.size == 0:
        return img_bgr

    # 1. Upscale if face region might be too small
    h, w = img_bgr.shape[:2]
    if max(h, w) < 600:
        scale = 600 / max(h, w)
    ...
    # 2. CLAHE on L-channel (LAB colour space)
    ...
    # 3. Unsharp mask — enhances fine detail (eyes, hair, skin texture)
...
def extract(data: bytes, doc_type: str) -> tuple[Any | None, dict]:
    ...
    # Enhance document image before detection — card photos are often faded/compressed
    enhanced = enhance_doc_image(image_bgr)

    faces = detect_faces(enhanced)
    if not faces:
        # fallback: try raw image in case enhancement hurt something
