...
from aav.pipeline.documents import enhance_doc_image
...
async def verify(request: Request, sid: str, body: VerifyRequest) -> dict:
    ...
    for data_url in body.frames:
        ...
        # Enhance live frame too — same pipeline as document side
        # so both embeddings are computed on similarly processed images
        img = enhance_doc_image(img)
    ...
    decision = decide_pct(similarity, age_gap)
